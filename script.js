 ...
 token = null;
  ...
  .catch((err) => {
    alert("Кажется, у вас упал интернет, попробуйте позже");
    console.warn(err);
  });
  };
  ...
  const answer = () => {
    ...
    const initEventLike = () => {
      ...
      const listComment = comments.map((user, index) => {
        const now = new Date();
        const createDate = format(now, "MM-dd-yyyy hh:mm"); // дата в новом формате
     ...
     buttonElement.addEventListener("click", () => {
      ...
      form.innerHTML = "Подождите комментарий добавляется";
